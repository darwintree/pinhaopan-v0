...
export function GuideListItem({
  ...
  const validTags = guide.tags
    .filter((tag) => tagList.some((t) => t.name === tag))
    .map((tag) => ({
      name: tag,
    ...
    <>
      <TableRow
        className={`cursor-pointer hover:bg-slate-100/50 dark:hover:bg-slate-800/50 ${
          isAnyLightboxOpen ? "pointer-events-none opacity-50" : ""
        }`}
        onClick={() => {
      ...
      >
        ...
        <TableCell className="px-3 py-1">
          {guide.time
            ? `${Math.floor(guide.time / 60)}:${(guide.time % 60)
                .toString()
                .padStart(2, "0")}`
            : "-"}
