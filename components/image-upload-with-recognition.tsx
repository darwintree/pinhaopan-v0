...
export function ImageUploadWithRecognition({
  ...
  const [modeData, setModeData] = useState<ModeData>({
    ...
    mask: {
      position: { x: 50, y: 50 },
      size: { width: 300, height: type === "chara" ? 300/getDefaultAspectRatio(type) : 169 },
  ...
  const handleModeChange = (newMode: RectangleMode) => {
    ...
    if (newMode === "individual") {
      ...
      if (modeData.mask.presetRectangles.length === 0 && imageRef.current) {
        ...
        // 计算默认蒙版位置和大小
        const defaultAspectRatio = getDefaultAspectRatio(type)
        const maskWidth = width * 0.8
        const maskHeight = maskWidth / defaultAspectRatio
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Switch 
              checked={mode === "mask"} 
              onCheckedChange={(checked) => handleModeChange(checked ? "mask" : "individual")} 
            />
          </div>
          {/* <div className="flex items-center gap-2">
